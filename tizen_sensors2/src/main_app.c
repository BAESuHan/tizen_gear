...
float sumRaw(float x, float y, float z){
	float sum_xyz=x+y+z;
	return sum_xyz;
}
...
//					time_in_mill = (timeVal.tv_sec)*100 + (timeVal.tv_usec)/1000;
//					time_in_mill = (timeVal.tv_usec)/1000;
					time_in_mill = clock()/1000;

				}

				char a[50];
				sum_accel[idx]=sumRaw(event->values[0],event->values[1],event->values[2]);
//				sprintf(a,"%.1f, %.1f, %.1f", event->values[0],event->values[1],event->values[2]);
				sprintf(a,"%.1f",sum_accel[idx]);
